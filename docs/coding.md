...
Regularly review and refactor your code to improve its structure, readability, and maintainability. Aim to leave the codebase cleaner than you found it.

**Rationale**: Code evolves, and initial designs may not hold up. Continuous refactoring prevents code decay and keeps the system manageable over time.
...
Nested conditional logic (e.g., deep `if`/`else if`/`else` chains) can be hard to read and test. Encapsulate such logic into separate functions with descriptive names.

```
// Bad: Nested logic within the main function
...
// Good: Logic encapsulated in a helper function
...
function processOrder(order) {
    let status = determineOrderStatus(order);
    // ... use status ...
}
```

**Rationale**: Isolating complex decision logic improves the readability of the calling function, makes the logic itself easier to understand and test, and promotes reusability (following the Single Responsibility Principle).
